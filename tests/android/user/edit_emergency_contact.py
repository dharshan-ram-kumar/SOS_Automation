...
def edit_emergency_contact(driver):
    try:
        edit_contact = (driver.find_element(MobileBy.ANDROID_UIAUTOMATOR, 'new UiSelector().text("").instance(2)'))
        edit_contact.click()
        time.sleep(2)

        name = (driver.find_element(MobileBy.ANDROID_UIAUTOMATOR,'new UiSelector().text("Test")'))
        name.clear()
        type_name = (driver.find_element(MobileBy.ANDROID_UIAUTOMATOR,'new UiSelector().text("Name")'))
        type_name.send_keys("Test1")
        time.sleep(2)

        relationship = (driver.find_element(MobileBy.ANDROID_UIAUTOMATOR, 'new UiSelector().text("Relationship")'))
        relationship.clear()
        ...
        number = (driver.find_element(MobileBy.ANDROID_UIAUTOMATOR, 'new UiSelector().text("9876543210")'))
        number.clear()
        ...
        save_contact = (driver.find_element(MobileBy.ANDROID_UIAUTOMATOR,'new UiSelector().description("Save")'))
        save_contact.click()
        time.sleep(2)
